 ...
 * }
    ...
    */
    ...
    /**
     * __kernel void MatVecMulUncoalesced0(const __global float* M, const __global float* V,
     * uint width, uint height, __global float* W) {
     *      // Row index
    ...
    */
    TEST_CASE("matrix_multiplication") {
        auto kernArgAddr = 0;
        std::vector<size_t> globalWorkSize{5, 5, 5};
        size_t dataSize =
            std::max(globalWorkSize[0], std::max(globalWorkSize[1], globalWorkSize[2]));
        std::vector<size_t> localWorkSize{5, 5, 5};
        const std::vector<std::string>& config = {
            ".config\n",
            ".dims x\n",
            ".sgprsnum 15\n",
        ...
        const std::vector<std::string>& instructions = {
            ...
            ".L160_0:",
            "/*0000000000a0*/ flat_load_dword v4, v[0:1]",
